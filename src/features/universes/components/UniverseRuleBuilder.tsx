...
interface UniverseRuleBuilderProps {
  value: UniverseDraftDefinition;
  onChange: (nextValue: UniverseDraftDefinition) => void;
}

function getNodeAtPath(root: UniverseDraftGroup, path: NodePath): UniverseDraftNode {
  let current: UniverseDraftNode = root;
...
function updateNodeAtPath(
  ...
  path: NodePath,
  updater: (node: UniverseDraftNode) => UniverseDraftNode
): UniverseDraftDefinition {
  const nextUniverse = cloneUniverse(universe);
  if (path.length === 0) {
    nextUniverse.root = updater(nextUniverse.root) as UniverseDraftGroup;
    return nextUniverse;
  }
...
function removeNodeAtPath(universe: UniverseDraftDefinition, path: NodePath): UniverseDraftDefinition {
  if (path.length === 0) {
    return universe;
  }
  const nextUniverse = cloneUniverse(universe);
...
function getCatalogField(
  ...
  return (
    catalog?.fields.find((field: UniverseFieldDefinition) => field.id === fieldId) || null
  );
}

function toMultiValueText(values: UniverseDraftCondition['values']): string {
...
function getPreviewFieldLabel(
  catalog: UniverseCatalogResponse | undefined,
  fieldId: string
): string {
  return getCatalogField(catalog, fieldId)?.label || fieldId;
}
...
export function UniverseRuleBuilder({ value, onChange }: UniverseRuleBuilderProps) {
  ...
  const catalogQuery = useQuery({
    queryKey: ['universes', 'catalog'],
    queryFn: () => universeApi.getUniverseCatalog()
  });

  const previewMutation = useMutation({
    mutationFn: (universe: ReturnType<typeof materializeUniverseDefinition>) =>
      universeApi.previewUniverse({
        universe,
        sampleLimit: 12
      }),
    onSuccess: (response) => {
  ...
  const updateUniverse = (nextUniverse: UniverseDraftDefinition) => {
  ...
  const updateCondition = (
    path: NodePath,
    updater: (condition: UniverseDraftCondition) => UniverseDraftCondition
  ) => {
    updateUniverse(
      updateNodeAtPath(value, path, (node) => {
        if (isUniverseGroup(node)) return node;
  ...
  const handleFieldChange = (path: NodePath, fieldId: string) => {
    const field = getCatalogField(catalogQuery.data, fieldId);
  ...
  const handleOperatorChange = (path: NodePath, operator: string) => {
    updateCondition(path, (condition) => ({
      ...condition,
      operator: (operator || '') as UniverseConditionOperator | '',
      value:
  ...
  const previewUniverse = () => {
    try {
      const materializedUniverse = materializeUniverseDefinition(value);
      previewMutation.mutate(materializedUniverse);
    } catch (error) {
      setPreview(null);
      setPreviewError(formatSystemStatusText(error) || 'Universe rules are incomplete.');
    }
  };

  const renderConditionValueEditor = (condition: UniverseDraftCondition, path: NodePath) => {
    ...
    if (!field) {
      return (
        <div className="rounded-lg border border-dashed border-border/70 bg-muted/15 px-4 py-3 text-xs text-muted-foreground">
          Select a field to configure the expected value type.
        </div>
      );
    }
  ...
  const renderNode = (node: UniverseDraftNode, path: NodePath = [], depth: number = 0) => {
    if (!isUniverseGroup(node)) {
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div className="grid gap-2">
              ...
              >
                ...
                {(catalogQuery.data?.fields || []).map((item: UniverseFieldDefinition) => (
            ...
            <div className="grid gap-2">
              ...
              <select
                ...
                className={selectClassName}
                value={node.operator}
                onChange={(event) => handleOperatorChange(path, event.target.value)}
                disabled={!selectedField}
              >
                <option value="">Select an operator</option>
                {availableOperators.map((operator: UniverseConditionOperator) => (
          ...
          {preview ? (
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {preview.fieldsUsed.map((fieldId: string) => (
                  <Badge key={fieldId} variant="outline">
                    {getPreviewFieldLabel(catalogQuery.data, fieldId)}
                  </Badge>
                ))}
              </div>
              ...
              {preview.sampleSymbols.length ? (
                <div className={mutedSurfaceClassName}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {preview.sampleSymbols.map((symbol: string) => (
