...
- `UI_AUTH_ENABLED` is a first-class repo or environment variable consumed at UI container startup. It can still be disabled for local-only bypass scenarios, but production deployments must set it to `true` whenever the proxied control-plane `/config.js` reports `authRequired=true`.
