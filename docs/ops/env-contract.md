...
- `API_UPSTREAM` is a first-class repo variable and falls back to the control-plane Container App FQDN when it can be discovered. Store the host only, without `https://`; the scheme is carried separately in `API_UPSTREAM_SCHEME`. In production, point it at the control-plane internal host or internal ACA FQDN rather than the public API hostname. `deploy-prod.yml`, `rollback-prod.yml`, and `deploy-ui-runtime.yml` read both values from repo vars only; there is no workflow input override.
- `API_UPSTREAM_SCHEME` is a first-class repo variable that controls how the UI proxy connects to `API_UPSTREAM`. Use the scheme required by the internal control-plane route and avoid the public API origin on the normal UI path.
...
- `UI_AUTH_PROVIDER=oidc` is the normal deployed mode whenever `UI_AUTH_ENABLED=true`. The standard deploy path no longer supports `password`.
