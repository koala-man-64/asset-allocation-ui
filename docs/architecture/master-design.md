...
This document is the living architecture contract for `asset-allocation-ui`. It records the repo's intended role, runtime model, dependency boundaries, operational contract, and known drift as of April 18, 2026.
...
- Confirmed: Normal dependency flow is UI -> published `@asset-allocation/contracts` package -> control-plane `/config.js`, `/api/*`, `/api/auth/session`, realtime ticketing, health endpoints, and OIDC-backed access. Evidence: `package.json`, `src/config.ts`, `nginx.conf`, `src/hooks/useRealtime.ts`, `src/app/components/auth/OidcAccessGate.tsx`.
...
| `src/features/*` | Route-facing feature layer for page-level behavior, helper ownership, and domain composition. | Confirmed | `src/app/routes.tsx`, `src/features/*`, `src/app/__tests__/*` |
| `src/services/*` | Transport and backend interaction seams, with `apiService` as the low-level transport and focused domain facades layered beneath the aggregate `DataService`. | Confirmed | `src/services/apiService.ts`, `src/services/apiTypes.ts`, `src/services/authDataService.ts`, `src/services/marketDataService.ts`, `src/services/systemDataService.ts`, `src/services/DataService.ts`, `src/services/authTransport.ts`, `src/services/PostgresService.ts`, `src/services/*Api.ts` |
...
| `src/app/components/pages/*` and related helpers | Residual legacy territory limited to a small set of orphaned pages/tests; active route shims and shared helper imports have been removed from the live feature surface. | Drift | `src/app/components/pages/*.tsx`, `src/features/*`, `src/app/__tests__/*` |
...
6. Confirmed: `src/contexts/AuthContext.tsx` owns the browser-auth coordinator: singleton MSAL session setup, callback handling, one-shot silent restore, explicit redirect start on user action, token acquisition, and background reauth prompting. Evidence: `src/contexts/AuthContext.tsx`, `src/contexts/msalSession.ts`, `src/services/authTransport.ts`.
7. Confirmed: `src/app/components/auth/OidcAccessGate.tsx` keeps the app shell visible while protected route content is gated and renders explicit sign-in, session-expired, redirect-in-progress, and access-check states before loading protected routes. It uses `DataService.getAuthSessionStatusWithMeta()` as the primary post-login access probe, with an explicit temporary fallback to `DataService.getSystemHealthWithMeta()` while older control-plane deployments roll forward. Evidence: `src/app/components/auth/OidcAccessGate.tsx`, `src/services/DataService.ts`.
...
| `/auth/logout-complete` | Finish explicit sign-out on a neutral signed-out route without re-entering the sign-in callback error path. | OIDC post-logout redirect plus local session cleanup. | `src/app/components/auth/OidcAccessGate.tsx`, `src/contexts/AuthContext.tsx`, `src/config.ts` | Confirmed |
...
| `/strategies` | Trading-desk-oriented strategy workspace for browse, inspect, create, duplicate, edit, delete, and secondary backtest launch flows. | Strategy and backtest endpoints via `strategyApi` and `backtestApi`. | `src/features/strategies/StrategyConfigPage.tsx`, `src/features/strategies/components/*`, `src/features/strategies/lib/*`, `src/services/strategyApi.ts`, `src/services/backtestApi.ts` | Confirmed |
...
- Confirmed: Runtime config is injected at runtime through `/config.js` and `window.__API_UI_CONFIG__`, not baked into the SPA bundle; that runtime payload includes the browser callback URI plus the derived post-logout completion URI. Evidence: `index.html`, `src/config.ts`, `nginx.conf`, `DEPLOYMENT_SETUP.md`.
...
- Confirmed: Browser auth and bearer-token handling stay centralized in `src/contexts/AuthContext.tsx`, `src/contexts/msalSession.ts`, and `src/services/authTransport.ts`, with lower-level transports raising reauth-required signals instead of starting redirects directly. Evidence: `src/contexts/AuthContext.tsx`, `src/contexts/msalSession.ts`, `src/services/authTransport.ts`, `src/contexts/__tests__/AuthContext.test.tsx`.
...
- Confirmed: New route-facing work should live under `src/features/*`; the legacy page re-export shims used by tests and routes have been removed from the active route surface. Evidence: `src/app/routes.tsx`, `src/features/*`, `src/app/__tests__/*`.
- Confirmed: New server-backed behavior should extend existing seams (`AuthContext`, `authTransport`, `apiService`, focused domain data services, query hooks, realtime hooks) rather than introduce feature-local `fetch`, feature-local token logic, or an additional global server-state library. Evidence: `src/contexts/AuthContext.tsx`, `src/services/authTransport.ts`, `src/services/apiService.ts`, `src/services/authDataService.ts`, `src/services/marketDataService.ts`, `src/services/systemDataService.ts`, `src/services/DataService.ts`, `src/providers/QueryProvider.tsx`, `src/hooks/useRealtime.ts`.
...
| Auth/session coordination and bearer token injection are centralized, and background `401`/`4401` failures raise a UI-owned reauth prompt instead of a surprise redirect. | `src/contexts/AuthContext.tsx`, `src/contexts/msalSession.ts`, `src/services/authTransport.ts`, `src/app/components/auth/OidcAccessGate.tsx`, `src/hooks/useRealtime.ts` | Confirmed | `src/contexts/__tests__/AuthContext.test.tsx`, `src/app/__tests__/App.auth.test.tsx`, `src/hooks/useRealtime.test.tsx`, `src/services/authTransport.test.ts` |
...
| Realtime is used for invalidation and log/event streaming and suppresses duplicate unauthorized noise while a shared reauth redirect is in flight. | `src/hooks/useRealtime.ts`, `src/services/realtimeBus.ts`, `src/services/authTransport.ts` | Confirmed | `src/hooks/useRealtime.test.tsx` |
...
| The route surface is feature-owned, with only a small residual set of unused legacy page modules still present outside the active route graph. | `src/app/routes.tsx`, `src/features/*`, `src/app/components/pages/*.tsx` | Confirmed | `ci.yml` build path |

## Known Drift and Transitional Structure

| Status | Area | Observed Current State | Intended Design | Risk | Next Cleanup Trigger |
| --- | --- | --- | --- | --- | --- |
| Drift | Route layer vs. legacy page tree | `src/app/routes.tsx` lazy-loads `src/features/*`, shared page shims were removed, and helper ownership for `system-status`, `data-quality`, and universe editing moved under `src/features/*`; a few unused legacy page files still remain under `src/app/components/pages/*`. | New route-facing logic should live in `src/features/*`, with the remaining orphan legacy files removed as follow-on cleanup. | Low but non-zero ownership confusion remains around the residual orphan files. | Any PR that touches `src/app/components/pages/*` or adds a new route. |
| Resolved (2026-04-18) | Tests relied on legacy import shims | Route-level tests now import feature entrypoints directly, and the one-line page re-export shims were deleted. | Tests target stable feature entrypoints or intentionally named compatibility shims. | Residual risk is low because the shims no longer anchor test paths. | Re-open only if a new compatibility shim is introduced. |
| Resolved (2026-04-18) | Shared helpers were buried in page paths | `system-status`, `data-quality`, and universe-editor helpers/components now live under feature-owned directories or neutral shared utilities. | Shared non-visual logic should live in `src/features/<domain>/...` or clearly neutral shared modules. | Reduced discovery and duplication risk. | Re-open only if new shared helpers are added under `src/app/components/pages/*`. |
| Resolved (2026-04-18) | Type ownership seam was mixed | Shared contract exports now live in `src/types/strategyContracts.ts`, while `src/types/strategy.ts` remains the stable barrel for local view models plus contract re-exports. | Shared business schemas should come from `@asset-allocation/contracts`; UI-local view models should stay clearly local and separate. | Ownership is clearer for future schema changes. | Re-open only if contract and local models start mixing again. |
...
corepack pnpm vitest run src/app/__tests__/App.auth.test.tsx src/contexts/__tests__/AuthContext.test.tsx src/services/__tests__/apiService.test.ts src/hooks/useRealtime.test.tsx src/services/authTransport.test.ts
```
...
| --- | --- | --- | --- |
| 2026-04-18 | Removed legacy page re-export shims, moved shared feature helpers out of `src/app/components/pages/*`, split API response types into `src/services/apiTypes.ts`, introduced focused domain data services under `src/services/*`, and extracted contract-owned strategy exports into `src/types/strategyContracts.ts`. | Reduce legacy ownership drift without changing routes, backend endpoints, env/runtime config, or published contracts. | `src/app/routes.tsx`, `src/app/__tests__/*`, `src/features/data-quality/*`, `src/features/system-status/*`, `src/features/universes/*`, `src/services/apiService.ts`, `src/services/apiTypes.ts`, `src/services/authDataService.ts`, `src/services/marketDataService.ts`, `src/services/systemDataService.ts`, `src/services/DataService.ts`, `src/types/strategy.ts`, `src/types/strategyContracts.ts`, `package.json`, `pnpm-lock.yaml` |
