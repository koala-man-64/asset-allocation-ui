...
| 2026-04-17 | Reworked the OIDC boundary around silent-first sign-in, `/api/auth/session`, logout-complete routing, and reauth-aware realtime transport. | Align the repo architecture contract with the seamless auth overhaul and keep UI docs in sync with the new control-plane surface. | `src/app/App.tsx`, `src/app/components/auth/OidcAccessGate.tsx`, `src/contexts/AuthContext.tsx`, `src/services/authTransport.ts`, `src/services/apiService.ts`, `src/hooks/useRealtime.ts`, `src/app/__tests__/App.auth.test.tsx`, `src/contexts/__tests__/AuthContext.test.tsx` |
