...
| Control-plane `/ui-config.js` | Runtime UI config injection before React boot. | Confirmed | `index.html`, `src/config.ts`, `docker/write-ui-runtime-config.sh`, `DEPLOYMENT_SETUP.md` |
...
6. Confirmed: `src/contexts/AuthContext.tsx` owns the browser-auth coordinator for password and cookie-session auth: login submission, session checks, login-route redirection, and logout coordination. Evidence: `src/contexts/AuthContext.tsx`, `src/services/DataService.ts`.
7. Confirmed: `src/app/components/auth/AuthPage.tsx` and `src/app/components/auth/RequireSession.tsx` keep the app shell protected by checking `/api/auth/session`, rendering the password prompt, and redirecting unauthorized users to `/login` before protected routes load. Evidence: `src/app/components/auth/AuthPage.tsx`, `src/app/components/auth/RequireSession.tsx`, `src/services/DataService.ts`.
...
| --- | --- | --- | --- | --- |
| `/auth/callback` | Temporary compatibility route that redirects stale OIDC links back to `/login`. | UI-owned compatibility shim during auth migration. | `src/app/components/auth/AuthPage.tsx`, `src/app/App.tsx` | Confirmed |
| `/auth/logout-complete` | Temporary compatibility route that lands the user on the signed-out password login screen. | UI-owned compatibility shim during auth migration. | `src/app/components/auth/AuthPage.tsx`, `src/app/App.tsx` | Confirmed |
...
| --- | --- | --- | --- |
| 2026-04-25 | Replaced the browser-side OIDC/MSAL flow with a password login screen, backend-issued cookie sessions, `/ui-config.js` auth-provider bootstrap, and ACA ingress allowlist rendering. | Move the UI to the approved private-only shared-password model while keeping same-origin API and realtime access. | `src/app/App.tsx`, `src/app/components/auth/AuthPage.tsx`, `src/app/components/auth/RequireSession.tsx`, `src/contexts/AuthContext.tsx`, `src/services/apiService.ts`, `src/hooks/useRealtime.ts`, `docker/write-ui-runtime-config.sh`, `deploy/app_ui.yaml`, `.github/workflows/deploy-ui-runtime.yml`, `docs/ops/env-contract.csv` |
