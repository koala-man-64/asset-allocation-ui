...
- Confirmed: Normal dependency flow is UI -> published `@asset-allocation/contracts` package -> UI-owned `/ui-config.js` bootstrap -> MSAL bearer token acquisition -> control-plane `/api/*`, `/api/auth/session`, realtime ticketing, and health endpoints. Evidence: `package.json`, `src/config.ts`, `index.html`, `docker/write-ui-runtime-config.sh`, `nginx.conf`, `src/hooks/useRealtime.ts`, `src/app/components/auth/AuthPage.tsx`, `src/app/components/auth/RequireSession.tsx`.
...
| `src/services/*` | Transport and backend interaction seams, including low-level bearer-auth HTTP, realtime browser event bus, and Postgres catalog access. | Confirmed | `src/services/apiService.ts`, `src/services/DataService.ts`, `src/services/PostgresService.ts`, `src/services/*Api.ts` |
...
| `src/contexts/*` | Cross-cutting browser session concerns, specifically login routing, OIDC session state, bearer token acquisition, and logout coordination. | Confirmed | `src/contexts/AuthContext.tsx`, `src/app/components/auth/AuthPage.tsx`, `src/app/components/auth/RequireSession.tsx` |
...
6. Confirmed: `src/contexts/AuthContext.tsx` owns the browser-auth coordinator for Entra OIDC bearer auth: login launch, callback completion, session checks, login-route redirection, silent token acquisition, and logout coordination. Evidence: `src/contexts/AuthContext.tsx`, `src/services/DataService.ts`, `src/services/oidcClient.ts`.
7. Confirmed: `src/app/components/auth/AuthPage.tsx` and `src/app/components/auth/RequireSession.tsx` keep the app shell protected by launching `/login` into Entra OIDC, checking bearer-backed `/api/auth/session`, and redirecting unauthorized users before protected routes load. Evidence: `src/app/components/auth/AuthPage.tsx`, `src/app/components/auth/RequireSession.tsx`, `src/services/DataService.ts`, `src/services/apiService.ts`.
...
| --- | --- | --- | --- | --- |
| `/auth/callback` | Completes the Entra redirect flow, stores the active MSAL account, and restores the saved deep link. | UI-owned OIDC callback boundary for direct bearer auth. | `src/app/components/auth/AuthPage.tsx`, `src/contexts/AuthContext.tsx`, `src/services/oidcClient.ts` | Confirmed |
...
- Confirmed: Browser auth coordination stays centralized in `src/contexts/AuthContext.tsx`, `src/app/components/auth/AuthPage.tsx`, `src/app/components/auth/RequireSession.tsx`, `src/services/apiService.ts`, and `src/hooks/useRealtime.ts`, with the UI attaching MSAL-acquired bearer tokens to API and realtime-ticket requests. Evidence: `src/contexts/AuthContext.tsx`, `src/app/components/auth/AuthPage.tsx`, `src/app/components/auth/RequireSession.tsx`, `src/services/apiService.ts`, `src/hooks/useRealtime.ts`, `src/contexts/__tests__/AuthContext.test.tsx`.
...
| Auth/session coordination is centralized, and background `401`/`4401` failures redirect the user back through OIDC rather than showing a password flow. | `src/contexts/AuthContext.tsx`, `src/app/components/auth/AuthPage.tsx`, `src/app/components/auth/RequireSession.tsx`, `src/hooks/useRealtime.ts`, `src/services/apiService.ts` | Confirmed | `src/contexts/__tests__/AuthContext.test.tsx`, `src/app/__tests__/App.auth.test.tsx`, `src/hooks/useRealtime.test.tsx`, `src/services/__tests__/apiService.test.ts` |
...
| Realtime is used for invalidation and log/event streaming and its ticket request uses the same bearer auth boundary as the rest of the UI. | `src/hooks/useRealtime.ts`, `src/services/realtimeBus.ts` | Confirmed | `src/hooks/useRealtime.test.tsx` |
...
| --- | --- | --- | --- |
| 2026-04-28 | Bumped the published `@asset-allocation/contracts` dependency to `3.11.0` and stamped UI release metadata with runtime-common `3.4.7`. | Keep the UI package and release manifest aligned with the current cross-repo version matrix without adding a runtime-common install dependency to the browser app. | `package.json`, `pnpm-lock.yaml`, `.github/workflows/release.yml`, `tests/test_multirepo_dependency_contract.py` |
| 2026-04-29 | Migrated Entra OIDC from backend cookie bootstrap to direct bearer auth and removed shared-password auth from the UI deploy path. | Align the UI with the OIDC-only control plane and make REST plus realtime-ticket calls explicitly bearer-authenticated. | `src/app/App.tsx`, `src/app/components/auth/AuthPage.tsx`, `src/contexts/AuthContext.tsx`, `src/services/oidcClient.ts`, `src/services/apiService.ts`, `src/hooks/useRealtime.ts`, `docker/write-ui-runtime-config.sh`, `.github/workflows/deploy-ui-runtime.yml`, `docs/ops/env-contract.csv` |
