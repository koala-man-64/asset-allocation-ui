...
| `/strategies` | Single strategy workspace for strategy library, universe/ranking/regime/risk editing, portfolio-backed allocations, trade history, historical evidence, forecasts, and comparison. | Strategy, universe, ranking, backtest, portfolio snapshot, and strategy analytics endpoints via `strategyApi`, `universeApi`, `rankingApi`, `backtestApi`, and `strategyAnalyticsApi`. | `src/features/strategies/StrategyConfigPage.tsx`, `src/features/strategies/components/*`, `src/features/strategies/lib/*`, `src/services/strategyApi.ts`, `src/services/strategyAnalyticsApi.ts`, `src/services/universeApi.ts`, `src/services/rankingApi.ts`, `src/services/backtestApi.ts` | Confirmed |
...
| `/universes` | Compatibility-only direct route for reusable universe definitions; removed from visible navigation because authoring is folded into `/strategies`. | Universe config endpoints via `universeApi`. | `src/features/universes/UniverseConfigPage.tsx`, `src/services/universeApi.ts` | Hidden compatibility |
| `/rankings` | Compatibility-only direct route for ranking schemas; removed from visible navigation because authoring is folded into `/strategies`. | Ranking, strategy, and universe endpoints. | `src/features/rankings/RankingConfigPage.tsx`, `src/services/rankingApi.ts`, `src/services/strategyApi.ts`, `src/services/universeApi.ts` | Hidden compatibility |
