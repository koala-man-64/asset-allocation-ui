...
| Drift | Broker account contracts are temporarily mirrored locally | `src/types/brokerAccounts.ts` mirrors pending broker-account shapes so the new `/accounts` route can land before the next published `@asset-allocation/contracts` release. | The `/accounts` surface should import these shapes from the published contracts package once the broker-account models are released. | Local mirrors can drift from the contracts repo if the bridge survives past the release it is waiting for. | The first `@asset-allocation/contracts` release that includes broker-account models. |
