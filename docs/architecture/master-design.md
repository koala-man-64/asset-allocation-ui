...
- Confirmed: The `/accounts` surface still uses a repo-local broker-account bridge because the latest published `@asset-allocation/contracts` package adopted by this repo does not yet export that surface. Evidence: `src/types/brokerAccounts.ts`, `src/services/accountOperationsApi.ts`, `package.json`.
...
| Drift | Broker account contracts are temporarily mirrored locally | `src/types/brokerAccounts.ts` still defines the broker-account shapes because the published `@asset-allocation/contracts` `3.0.0` package does not yet export those models. | The `/accounts` surface should import these shapes from the published contracts package once the broker-account models are released there. | Local mirrors can drift from the contracts repo if the bridge survives past the release it is waiting for. | The first published `@asset-allocation/contracts` release that includes broker-account models. |
