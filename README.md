...
- `deploy-prod.yml` auto-deploys successful `UI Release` runs on `main` and redeploys only the current main release when run manually or when `asset-allocation-control-plane` dispatches `control_plane_released`.
- `rollback-prod.yml` deploys a specific prior UI image digest plus its release SHA to prod.
